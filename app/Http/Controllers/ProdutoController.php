...
public function movimentacao($id){
    ...
    if(in_array($item->tipo_transacao, ['os_consumo_peca', 'os_estorno_peca'], true)){
        $po = ProdutoOs::find($item->codigo_transacao);
        if($po){
            return redirect()->route('ordem-servico.show', [$po->ordem_servico_id]);
        }
    }
    if(in_array($item->tipo_transacao, ['reparo_interno_consumo_peca', 'reparo_interno_estorno_peca'], true)){
        $ln = ReparoInternoLinhaProduto::find($item->codigo_transacao);
        if($ln){
            return redirect()->route('reparo-interno.show', [$ln->reparo_interno_id]);
        }
    }
    if($item->tipo_transacao === AssistenciaEstoqueAjusteManual::TIPO_MOVIMENTACAO){
        $aj = AssistenciaEstoqueAjusteManual::find($item->codigo_transacao);
        if($aj && isset($aj->empresa_id) && (int)$aj->empresa_id === (int)request()->empresa_id){
            return redirect()->route('assistencia-estoque-ajuste.show', [$aj->id]);
        }
    }
}
