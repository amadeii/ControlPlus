            ...
            $nfe = DB::transaction(function () use ($request) {
                ...
                for ($i = 0; $i < sizeof($request->produto_id); $i++) {
                    ...
                    $itemNfe = ItemNfe::create([
                        ...
                        'perc_icms' => __convert_value_bd($request->perc_icms[$i] ?? 0),
                        'perc_pis' => __convert_value_bd($request->perc_pis[$i] ?? 0),
                        'perc_cofins' => __convert_value_bd($request->perc_cofins[$i] ?? 0),
                        'perc_ipi' => __convert_value_bd($request->perc_ipi[$i] ?? 0),
                        'cst_csosn' => $request->cst_csosn[$i] ?? $product->cst_csosn,
                        'cst_pis' => $request->cst_pis[$i] ?? $product->cst_pis,
                        'cst_cofins' => $request->cst_cofins[$i] ?? $product->cst_cofins,
                        'cst_ipi' => $request->cst_ipi[$i] ?? $product->cst_ipi,
                        'perc_red_bc' => !empty($request->perc_red_bc[$i]) ? __convert_value_bd($request->perc_red_bc[$i]) : 0,
                        'cfop' => $request->cfop[$i] ?? $product->cfop_entrada_estadual ?? $product->cfop_estadual,
                        'ncm' => $request->ncm[$i] ?? $product->ncm,
