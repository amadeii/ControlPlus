        ...
        <div class="auth-fluid-form-box">
            <div class="card-body d-flex flex-column h-100 gap-3">
                    ...
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="mb-3">
                            <input value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror"
                                type="text" id="name" placeholder="Nome" required name="name">
                            @error('name')
                        ...
                        <div class="mb-3">
                            <input value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror"
                                type="email" id="email" placeholder="Email" required name="email">
                            @error('email')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        @if ($captchaa == 1)
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('captcha') is-invalid @enderror" type="text"
                                    id="captcha" placeholder="Caracteres" required name="captcha">
                            </div>
                        @endif
                ...
                <footer class="footer footer-alt">
                    <p class="text-muted">Já tem conta? <a href="{{ route('login') }}"
