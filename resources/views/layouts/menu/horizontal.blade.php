        ...
        <nav class="navbar navbar-expand parent">
            <div class="collapse navbar-collapse active" id="topnav-menu-content">
                <ul class="navbar-nav">
                    ...
                    @if (__isActivePlan(Auth::user()->empresa, 'Serviços'))
                        @canany(['ordem_servico_view', 'ordem_servico_create', 'produtos_view', 'metas_view', 'reparo_interno_view', 'reparo_interno_create', 'assistencia_estoque_ajuste_view', 'assistencia_estoque_ajuste_create'])
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle arrow-none" href="#" id="assistencia-tecnica-menu"
                                    role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <i class="ri-customer-service-2-line"></i>
                                    <div class="arrow-down"></div>
                                </a>
                                <div class="dropdown-menu" aria-labelledby="assistencia-tecnica-menu">
                                    <label>Assistência técnica</label>
                                    @can('ordem_servico_view')
                                        <a href="{{ route('ordem-servico.index') }}" class="dropdown-item">Todas as OS</a>
                                        @php
                                            $__cfgOs = \App\Models\ConfigGeral::where('empresa_id', request()->empresa_id)->first();
                                            $__assistTpl = $__cfgOs && $__cfgOs->tipo_ordem_servico === 'assistencia técinica';
                                        @endphp
                                        @if($__assistTpl)
                                        <a href="{{ route('ordem-servico.fila-tecnica') }}" class="dropdown-item">Fila técnica</a>
                                        <a href="{{ route('ordem-servico.painel-assistencia') }}" class="dropdown-item">Painel assistência</a>
                                        @endif
                                    @endcan
                                    ...
                                    @can('produtos_view')
                                        <a href="{{ route('produtos.index') }}" class="dropdown-item">Peças</a>
                                    @endcan
                                    ...
                                    @can('reparo_interno_view')
                                        <a href="{{ route('reparo-interno.index') }}" class="dropdown-item">Reparo interno</a>
                                    @endcan
                                    @can('reparo_interno_create')
                                        <a href="{{ route('reparo-interno.create') }}" class="dropdown-item">Novo reparo interno</a>
                                    @endcan

                                    @can('assistencia_estoque_ajuste_view')
                                        <a href="{{ route('assistencia-estoque-ajuste.index') }}" class="dropdown-item">Baixas manuais (perdas)</a>
                                    @endcan
                                    @can('assistencia_estoque_ajuste_create')
                                        <a href="{{ route('assistencia-estoque-ajuste.create') }}" class="dropdown-item">Nova baixa manual</a>
                                    @endcan
                    ...
                    @if (__isActivePlan(Auth::user()->empresa, 'Pessoas'))
                        ...
                        @canany(['funcionario_view', 'apuracao_mensal_view', 'logs_view'])
                            <li class="nav-item dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="apuracao-menu">
                                    ...
                                    @can('logs_view')
                                        <a href="{{ route('auditoria-operacional.index') }}" class="dropdown-item">Auditoria operacional</a>
                                    @endcan
                                </div>
                            </li>
                        @endcanany
