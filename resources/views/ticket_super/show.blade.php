        ...
        <div class="card-body">
            <div class="row container">
                <div class="col-md-8">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9 col-12 mt-2">
                                ...
                                @foreach ($m->anexos as $key => $f)
                                    <a target="_blank" href="{{ $f->file }}">Conteudo anexado
                                        {{ $key + 1 }}</a><br>
                                @endforeach
                            </div>

                            @if ($item->status != 'resolvido')
                                <form class="col-12" action="{{ route('ticket-super.destroy-mensagem', $m->id) }}"
                                    method="post" id="form-{{ $m->id }}">
                                    @method('delete')
                    ...
                    @if ($item->status != 'resolvido')
                        <form class="row g-2" method="post" action="{{ route('ticket-super.add-mensagem', [$item->id]) }}"
                            enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="col-md-12">
                                {!! Form::textarea('descricao', 'Descrição')->attrs(['rows' => '10', 'class' => 'tiny']) !!}
                            </div>

                            <div class="col-md-8">
                                <label>Anexo</label>
                                <div class="input_container">
                                    {!! Form::file('anexos[]', '')->attrs(['multiple' => 'true']) !!}
                                </div>
                            </div>
                ...
                <div class="col-md-4">
                    <div class="card" style="background: #F7F7F7">
                        <div class="card-body">
                            ...
                            {!! Form::open()->fill($item)->put()->route('ticket-super.update-status', [$item->id]) !!}
                            <div class="row">
