        ...
        <div class="row">
            <h4 style="text-align:center; margin-top: -50px;">
                @if(($tipoDocumento ?? '') === 'entrada')
                    Termo de Entrada - OS #{{ $ordem->codigo_sequencial }}
                @elseif(($tipoDocumento ?? '') === 'final')
                    Termo de Encerramento - OS #{{ $ordem->codigo_sequencial }}
                @else
...
<body>
    ...
    @endif
        ...
        <table>
            <tr>
                ...
                <td class="b-top text-left" style="width: 180px;">
                    Marca: <strong>{{ $ordem->marca_equipamento ?: '—' }}</strong>
                </td>
                <td class="b-top text-left" style="width: 180px;">
                    Modelo: <strong>{{ $ordem->modelo_equipamento ?: '—' }}</strong>
                </td>
                <td class="b-top text-center" style="width: 200px;">
                    IMEI / Série: <strong>{{ $ordem->numero_serie ?: '—' }}</strong>
                </td>
                <td class="b-top text-right" style="width: 140px;">
        ...
        <table>
            <tr>
                <td class="b-top text-left" style="width: 500px;">
                    Acessórios: <strong>{{ $ordem->acessorios ?: '—' }}</strong>
                </td>
                @if(($tipoDocumento ?? '') === 'entrada')
                <td class="b-top text-right" style="width: 200px;">
                    Senha: <strong>{{ $ordem->senha_aparelho ?: '—' }}</strong>
                </td>
                @else
                <td class="b-top text-right" style="width: 200px;">
                    Senha: <strong>não exibida</strong>
                </td>
                @endif
            </tr>
        </table>
        ...
        <table>
            <tr>
                <td class="b-top b-bottom" style="width: 700px; height: 35px; text-align: left;">
                    <strong>CHECKLIST FÍSICO DE ENTRADA</strong>
                </td>
            </tr>
        </table>
        <table>
            <thead>
                <tr>
                    <th style="width: 180px; text-align: left;">Item</th>
                    <th style="width: 180px; text-align: left;">Estado</th>
                    <th style="width: 340px; text-align: left;">Observação</th>
                </tr>
            </thead>
            <tbody>
                @foreach($ordem->assistenciaChecklistFisicoItens as $chkFisicoPdf)
                <tr>
                    <td style="text-align: left">{{ $chkFisicoPdf->titulo }}</td>
                    <td style="text-align: left">{{ \App\Models\OrdemServico::assistenciaChecklistFisicoEstados()[$chkFisicoPdf->estado] ?? $chkFisicoPdf->estado }}</td>
                    <td style="text-align: left">{{ $chkFisicoPdf->observacao ?: '—' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <table>
            <tr>
                <td class="b-top b-bottom" style="width: 700px; height: 35px; text-align: left;">
                    <strong>FOTOS DO APARELHO</strong>
                </td>
            </tr>
        </table>
        <table>
            <tr>
                @foreach($ordem->anexos as $anexoPdf)
                    @php
                        $pathFotoOs = public_path($anexoPdf->caminho);
                    @endphp
                    @if($anexoPdf->caminho && file_exists($pathFotoOs))
                    <td style="width: 170px; text-align: center;">
                        <img style="max-width: 140px; max-height: 120px;"
                            src="{{ 'data:' . ($anexoPdf->mime ?: 'image/jpeg') . ';base64,' . base64_encode(file_get_contents($pathFotoOs)) }}">
                        <br>
                        <small>{{ $anexoPdf->tipo }}</small>
                    </td>
                    @endif
                @endforeach
            </tr>
        ...
        <tbody>
            @forelse($ordem->itens as $item)
                <tr>
                    ...
                    <td style="text-align: left">
                        {{ __moeda($item->quantidade != 0 ? ($item->subtotal / $item->quantidade) : 0) }}
                    </td>
                    <td style="text-align: left">
                        {{ __moeda($item->subtotal) }}
                    </td>
    ...
    @if(($tipoDocumento ?? '') === 'final')
    <table>
        <tr>
            <td class="b-top b-bottom" style="width: 700px; height: 35px; text-align: left;">
                <strong>GARANTIA E ENCERRAMENTO</strong>
            </td>
        </tr>
    </table>
    <table>
        <tr>
            <td class="b-top text-left" style="width: 350px;">
                Data de finalização: <strong>{{ isset($dataFinalizacaoDocumento) && $dataFinalizacaoDocumento ? __data_pt($dataFinalizacaoDocumento, true) : date('d/m/Y H:i') }}</strong>
            </td>
            <td class="b-top text-left" style="width: 350px;">
                Status final: <strong>
                    @if($configGeral && $configGeral->tipo_ordem_servico == 'assistencia técinica' && $ordem->estado === 'rp')
                        Sem reparo
                    @else
                        {{ \App\Models\OrdemServico::estados()[$ordem->estado] ?? $ordem->estado }}
                    @endif
                </strong>
            </td>
        </tr>
    ...
    <table>
        <tr>
            <td class="b-top text-left" style="width: 700px;">
                Garantia: <strong>conforme prazo cadastrado nos serviços e peças vinculados à OS.</strong>
            </td>
        </tr>
    </table>
    @endif
